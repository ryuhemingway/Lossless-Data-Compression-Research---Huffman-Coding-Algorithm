...
When analysing the time and space complexity of Huffman's algorithm, two parameters dictate the result. Let n denote the number of distinct symbols in the source alphabet, and m the total number of symbols in the input. For byte oriented input, n cannot exceed 256, since a byte represents at most 256 distinct values, whereas m grows without bound as the file grows. For any input of non trivial length it follows that n ≪ m, and the two parameters must therefore be kept distinct since they appear in different terms of the cost.
...
Counting and encoding only take one pass over the data, so each one costs Θ(m). Building the tree costs O(n log n), giving a total of Θ(m + n log n).

The cost of the tree comes from merging the loop. Each merge needs the two smallest remaining weights, then the heap finds the smallest in O(log n) time. For building a tree of n leaves and n-1 internal nodes, it will take 2n-1 insert and 2n-1 extractMin operations which with a binary heap gives O(n log n)[1,3]. An important point to note is that the log factor comes from sorting, not merging. Van Leeuwen proved this by showing the tree can be built in linear time if the weights are already sorted since internal nodes are created and consumed in non-decreasing weight order[3,5]. His technique is called the Two-Queue Method.

### Space Complexity:

In terms of space complexity, every merge removes two nodes from the queue and returns one, so the number of live nodes falls by exactly one per merge. Reducing n leaves to a single root therefore requires n−1 merges. Since each merge creates one internal node, the finished tree contains n leaves and n−1 internal nodes, resulting in 2n−1 in total. Each node stores a weight with two child pointers and constant number of words, so the tree occupies Θ(n) space. The same mechanism bounds the heap. It begins with n entries and never grows beyond that. The frequency table holds one counter per distinct symbol, also n. Every structure is therefore linear in the alphabet size, and total working space is Θ(n). Because m does not appear in this bound, the memory the algorithm itself needs is governed by alphabet size rather than input length, and in principle neither pass requires the input to be resident in full.

## General analysis of the algorithm/datastructure:

The Huffman algorithm minimises total encoded length. This is best represented by the function: Σ f(s) · L(s). Where f(s) is the frequency of symbol s and L(s) the length in bits of its codeword. Huffman states the objective in normalised form, as the average message length Lav = Σ P(i)L(i) over message probabilities P(i). He defines the target in his own words as "an ensemble code which, for a message ensemble consisting of a finite number of members, N, and for a given number of coding digits, D, yields the lowest possible average message length" [2]. The two formulations differ only by the constant factor m, so minimising either minimises the other. The algorithm is also greedy: at each step it merges the two lowest-weight nodes available, never reconsidering an earlier choice [3, 4]. Greedy strategies ordinarily yield only approximations. Writing of the earlier Shannon and Fano methods, Huffman notes that "their coding procedures are not optimum, but approach the optimum behavior when N approaches infinity" [2].

Huffman's greedy choice is the unusual case in which the strategy is provably optimal, and proving it relies on two properties [1]. The first is the greedy-choice property: some optimal code places the two lowest-weight symbols as sibling leaves at maximum depth. Huffman establishes this by an exchange argument, stating that "for an optimum code, the length of a given message code can never be less than the length of a more probable message code", because "if this requirement were not met, then a reduction in average message length could be obtained by interchanging the codes for the two messages in question in such a way that the shorter code becomes associated with the more probable message" [2]. Because the interchange merely permutes an existing assignment rather than altering the set of codewords, the prefix property is preserved. Also because the improvement is strict, *more frequent implies shorter* is a necessary condition for optimality rather than a design heuristic. 

Huffman derives two further necessary conditions. One is that "for an optimal code it is necessary that L(N) be equal to L(N − 1)"[2], where he orders the N messages by falling probability so that L(N) is the length of the code given to the least probable of them. The other is his restriction (d), that "at least two and not more than D of the messages with code length L(N) have codes which are alike except for their final digits", where D is the number of distinct coding digits, which is two for the binary case built here [2]. The second property is optimal substructure: replacing the merged pair with a composite symbol of summed weight produces an instance one symbol smaller, whose optimal solution extends to an optimal solution of the original. Huffman builds this into the procedure itself. The ensemble in which a composite replaces the two messages it was formed from "will be called the first auxiliary message ensemble", and since "each new auxiliary ensemble contains one less message than the preceding ensemble", the procedure "is applied again and again until the number of members in the most recently formed auxiliary message ensemble is reduced to two" [2]. He then shows that the necessary conditions are also sufficient: "when there is no alternative in choosing the two least probable messages, then it is clear that the requirements, established as necessary, are also sufficient for deriving an optimum code", and after dealing with ties between equally likely messages he concludes that "the procedure given is always sufficient to establish an optimum binary code" [2].

## Empirical Analysis

Two experiments were run to test whether the behaviour predicted in the analysis above actually appears when the algorithm is run. The first experiment focused on varying the total number of symbols m while holding the alphabet approximately constant. This was done in order to isolate the Θ(m) term. For the second experiment, m is fixed at 10,000 characters and only the shape of the symbol distribution varies, in order to test the relationship between compression and entropy. Both are driven by `empirical_analysis.py` with the random seed fixed at 42, so the generated inputs are identical on every run and the size, ratio, entropy and codeword length columns reproduce exactly. The timing columns do not, since they are wall clock measurements and each size is timed only once rather than averaged over repeated trials. The raw measurements are committed as `output/time_vs_size.csv` and `output/compression_ratio.csv`. 
...
An important point to note is that from 1,000 characters upwards decoding runs between 2.3 and 2.9 times slower than encoding, with the gap broadly widening as the input grows. The two smallest inputs do not follow this pattern. At 500 characters the gap is only 1.7 times, and at 100 characters decoding is actually faster than encoding, which is why the two curves in Figure 1 cross. This is a property of the implementation rather than of the algorithm itself. Encoding only performs one dictionary lookup per character, whereas decoding has to walk the tree one step for every bit, and since the average codeword is roughly six bits long this works out at six times as many steps for each character that is recovered. The measured gap is smaller than that factor of six because a single tree step is much cheaper than a dictionary lookup followed by a string join, and because both measurements also include the bit packing and unpacking work, which is common to the two directions.
...
Reading down the chart, the frequency of the symbols falls but the length of their codewords never falls with it. This is exactly what the exchange argument requires because more frequent symbols get shorter codewords while less frequent ones get longer ones. The space character appears 6 times and is given a 3 bit code, whereas every symbol that appears only once is given 5 bits. The average codeword length works out at 3.972 bits against an entropy of 3.933 bits, so the redundancy here is only 0.039 bits. This should also be compared against the eight bits per character that a fixed-width encoding would spend on the same string.

## Application

Huffman coding was developed to have lossless compression but it is not used for this case on its own. Typically instead, it is used in conjunction with other algorithms and techniques to optimise compression. When it is used in practice, it is typically used as the final entropy coding stage of a longer, more drawn-out and complex pipeline. In many cases, the earlier stage is typically a process that is geared towards removing the structural redundancy[3] and Huffman's algorithm is applied to the resulting symbols. This multistage modeling process has been used for a while now because it allows a single entropy coder to serve many different kinds of data[8]. 

The reason multiple algorithms have to be used is because entropy measured over single symbols does not take into account repeated symbols. Therefore repeated symbols looks no more compressible to Huffman than a random shuffle of the same characters. Shannon showed this directly when he compared successive approximations to English, finding that estimates conditioned on preceding letters give substantially lower entropy than estimates based solely on letter frequency [7]. A modelling stage that finds the repetition first, and an entropy coder that then exploits the frequency imbalance in what is left, therefore achieve considerably more together than either does on its own.

Where we use Huffman's coding algorithm daily is in ZIP archives, gzip, and the PNG image format. These all use the DEFLATE algorithm, whose last step is Huffman coding [9]. DEFLATE works in two stages. In the first stage, LZ77, repeated text is replaced by a backward reference: in "the cat sat on the mat, the cat sat on the rug", the second occurrence of "the cat sat on the " is replaced by an instruction to go back twenty four characters and copy the next nineteen. In the second stage, Huffman coding assigns short codewords to common symbols remaining after substitution and long codewords to rare ones. The two stages target distinct forms of waste: LZ77 removes repetition, which depends on character order, while Huffman eliminates frequency imbalance, which depends on symbol counts alone. This division of labour explains why Huffman coding alone misses repetition entirely: as shown in the empirical analysis, its view of the data extends no further than symbol frequency.

HTTP/2 gets around the header cost in a different way. Its header compression format, HPACK, encodes header text with a Huffman table that is fixed in the specification rather than built fresh for each message, so no tree has to be sent alongside the data [10]. This matters because HTTP headers are usually only a few hundred bytes, which the break even result above shows is far too small to carry a tree of its own.

## Implementation

Implementation of the Huffman coding algorithm was completed in Python. I chose Python because it was the first programming language that I learned and feel most comfortable with the logic and debugging of Python. Even though C is probably better to code in for this assignment due to being easier to manipulate in terms of memory, I felt that Python would allow me to complete the assignment most efficiently. To implement Huffman's coding algorithm I created `huffman_coding.py`, tested it with `test_huffman.py`, and did the empirical measurements in `empirical_analysis.py`. The graphs and analysis were produced from the resulting CSV files with the results of running the algorithm. Also, in terms of libraries, there weren't any third party libraries that were necessary to complete the setup and run the code. 

I encountered two difficulties implementing Huffman coding. First, handling a single distinct symbol: since a tree built from a single symbol has no internal nodes and the root is itself the leaf, the walk from root to leaf takes zero steps and the symbol receives the empty codeword. Encoding ten thousand identical characters produces a bit string of length zero. Although the tree fully describes the data, the decoder has no way of knowing how many characters to emit, which is why the header stores the original character count and the decoder rebuilds the run from it. Second, storing the tree as a pre-order traversal keeps the decoder simple but is not compact, costing 53 bytes for the 36-character sample, which is most of the reason small files fail to compress.
...
In summary, the results of Huffman's coding algorithm show that time scales linearly with input length, settling at 0.158 µs per character for encoding and 0.463 µs for decoding. Over a tenfold increase in input size, measured growth was a factor of 8.8x and 9.9x against a predicted factor of 10, matching the Θ(m) behaviour theory describes. Compression depends on source entropy rather than file size, and the guarantee that average codeword length satisfies H ≤ L < H + 1 held across all distributions tested, with five of six coming within 0.07 bits of the entropy floor. The two results that surprised me were both limitations. Below roughly a thousand characters, the compressed file comes out larger than the original because storing the decoding tree costs 53 bytes for the 36-character sample. Where a source has very few symbols, the code cannot allocate less than one whole bit per symbol, so a distribution split nine to one and a distribution split evenly compress to the exact same 1,267 bytes despite the first carrying less than half the information of the second.
...
[7] Claude E. Shannon. 1948. A mathematical theory of communication. The Bell System Technical Journal 27, 3 (July 1948), 379-423. https://doi.org/10.1002/j.1538-7305.1948.tb01338.x

[8] Jorma Rissanen and Glen G. Langdon. 1981. Universal modeling and coding. IEEE Transactions on Information Theory 27, 1 (January 1981), 12-23.

[9] L. Peter Deutsch. 1996. DEFLATE Compressed Data Format Specification version 1.3. RFC 1951. Internet Engineering Task Force. https://doi.org/10.17487/RFC1951

[10] Roberto Peon and Herve Ruellan. 2015. HPACK: Header Compression for HTTP/2. RFC 7541. Internet Engineering Task Force. https://doi.org/10.17487/RFC7541
